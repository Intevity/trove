...
| **claude-desktop** | R:PASS Q:PASS◑ | R:PASS Q:PASS◔    | R:PASS Q:PASS◑ | R:PASS Q:PASS◍ | R:PASS Q:PASS◑   | R:PASS Q:PASS◑ | ⊕              | —            | —         | —             | —       | —         | —          | —         | —             | —                | —            | —           |
| **cursor-ide**     | R:PASS Q:PASS◑ | R:PASS Q:PASS◔    | R:PASS Q:PASS◑ | R:PASS Q:PASS◍ | R:PASS Q:PASS◑   | R:PASS Q:PASS◑ | R:PASS Q:PASS◑ | —            | —         | —             | —       | —         | —          | —         | —             | —                | —            | —           |
| **cline**          | R:PASS Q:PASS◑ | R:PASS Q:PASS◔    | R:PASS Q:PASS◑ | R:PASS Q:PASS◔ | R:PASS Q:PASS◑   | R:PASS Q:PASS◑ | R:PASS Q:PASS◑ | —            | —         | —             | —       | —         | —          | —         | —             | —                | —            | —           |
...
| **opencode**       | R:PASS Q:FAIL◑ | R:PASS Q:PASS◇    | R:PASS Q:FAIL◑ | R:PASS Q:PASS◔ | R:PASS Q:FAIL◑   | R:PASS Q:FAIL◑ | R:PASS Q:FAIL◑ | —            | —         | —             | —       | —         | —          | —         | —             | —                | —            | —           |
...
- `⊛` cline not installed on the dev host (no `cline` entry in `state.json`; no `~/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/` directory). The cline row is SKIP across every column until VS Code + the Cline extension are present and Trove has applied the adapter. Not a Trove bug. _Superseded 2026-05-28 by the `cline-cli` adapter (Cline CLI 3.0.13, `npm i -g cline`, sessions under `~/.cline/data/sessions/<sid>/<sid>.messages.json`); see `◔`._
- `◔` 2026-05-28 follow-up pass — three cells filled in this commit. (a) **Bug K re-verification** via Trove apply: opencode plugin npm-install hook (`ensure_otel_plugin_installed` at `adapters/opencode.rs:97`) now succeeds on GUI-launched Trove because `probe_path` walks `~/.nvm/versions/node/*/bin`, `~/.volta/bin`, and `~/.local/share/fnm/node-versions/*/installation/bin` in addition to Homebrew. End-to-end against signoz: 323 spans + 7 logs with `service.name=opencode`, `harness.id=opencode`. Metrics didn't flush on a one-shot `opencode run` because the OTel-JS SDK default `OPENCODE_OTLP_METRICS_INTERVAL=60000` exceeds the CLI's lifetime; traces compensate. (b) **Three GUI rows × openobserve-local** — `claude-desktop` (3 `trove.harness.*` metric samples with `harness.id=claude-desktop`), `cursor-ide` (5 log rows with `service.name=cursor`, `harness.id=cursor-cli` confirming the Bug-L fix from `2e29e18` holds against a fresh openobserve), `cline-cli` (2 log rows + 1 `trove.harness.events` with `service.name=cline`, `harness.id=cline`). (c) **`cline-cli` adapter** for Cline 3.0.13's npm-installed `cline` binary — sessions live at `~/.cline/data/sessions/<sid>/<sid>.messages.json` (schema `{version, updated_at, agent, sessionId, messages[], system_prompt}`), distinct from the VS Code extension's `ui_messages.json` shape. The watcher tails both trees in parallel; payloads share `cline.task_id` but carry `cline.variant=cli|extension` so downstream rules can disambiguate. The four `tests/cline_e2e.rs` + `adapters/cline_watcher.rs::tests::cli_watcher_*` regression-locks cover the new path.
...
- `◑` 2026-06-01/02 strict one-stack-at-a-time fill session — signoz + openobserve taken down first, then one local stack up at a time per the Docker memory budget; fills the operator-driven GUI rows (claude-desktop, cursor-ide) plus cline-cli and opencode across grafana-local, opensearch-local, and the heavier stacks. On every stack, **claude-desktop** (metrics / `signaltometrics/cowork`), **cursor-ide** (logs, `harness.id=cursor-cli` — Bug-L mapping holds), and **cline-cli** (logs, `harness.id=cline`) PASS. **opencode is the recurring `Q:FAIL`**: on short turns it is traces-only (emits no logs/metrics), and its spans don't reach a queryable state on backends that index by span start-time or mangle trace service names. grafana-local's Tempo _ingests_ the spans (`tempo_distributor_spans_received_total=12`, `traces_created=6`) but can't time-search them — a skewed span start-timestamp in `@devtheops/opencode-plugin-otel` that only span-time-indexed stores expose; opensearch/Data Prepper lands the spans in `otel-v1-apm-span-*` with an empty `serviceName` (the `☆` drop) and Trove's opensearch trace delivery mostly `send_failed`s. opencode PASSes only where the store indexes by ingest time (openobserve `◇`, signoz `◔`). Not a Trove defect — per-stack receipts/queries in the dated run log.
...
### 2026-05-28 — Bug-K re-verify + three GUI rows × openobserve + `cline-cli` adapter (◔)

- **Trove preset config:** signoz-local stayed enabled from the `◍` run; openobserve-local re-enabled mid-session at `http://localhost:5080` (admin@example.com / changeme123). Both backends fan out simultaneously, so each smoke lands in two cells at once. Build: branch `fix/post-2026-05-22-matrix-followups` on top of `2e29e18`, rebuilt + reinstalled mid-session after the probe.rs + opencode.rs + cline_watcher.rs changes (per the schemaVersion-rebuild rule, even though schemaVersion didn't move — adapter behaviour changes still need a fresh `tauri build` to propagate).
- **Bug K npm-install hook — end-to-end verification:**
  - `ensure_otel_plugin_installed` (`adapters/opencode.rs:97`) previously fell over on GUI-launched Trove because `probe_path` only walked `$PATH` + Homebrew dirs; macOS launchd hands GUI apps a minimal PATH (`/usr/bin:/bin:/usr/sbin:/sbin`), and the user's `npm` lives at `~/.nvm/versions/node/v24.14.0/bin/npm` — invisible. Extended `probe::fallback_bin_dirs` (renamed from `homebrew_fallback_dirs`) to also walk `~/.nvm/versions/node/*/bin` (mtime-desc — newest install wins on tie), `~/.volta/bin`, and `~/.local/share/fnm/node-versions/*/installation/bin`. After re-toggling opencode in the UI, `~/.config/opencode/node_modules/@devtheops/opencode-plugin-otel/package.json` materialises with an mtime matching the re-enable timestamp.
  - Telemetry round-trip against signoz: `OPENCODE_ENABLE_TELEMETRY=1 OPENCODE_OTLP_ENDPOINT=http://127.0.0.1:4318 OPENCODE_OTLP_PROTOCOL=http/protobuf opencode run "say hi briefly"`. SigNoz ClickHouse: 7 log rows + 323 trace spans with `service.name=opencode` / `harness.id=opencode` within 60s. Metrics did NOT flush because the OTel-JS SDK's default `OPENCODE_OTLP_METRICS_INTERVAL=60000` is longer than opencode's one-shot CLI lifetime; traces+logs (5s log batch, on-shutdown trace flush) compensate. Not a Trove bug.
  - **Trove-side env-var injection is intentionally out of scope** per the opencode adapter docstring (`adapters/opencode.rs:31`): the plugin's `OPENCODE_*` env vars are configured in the user's shell, not by Trove's config file (opencode.json has no `env` block — verified against `https://opencode.ai/docs/config/`). The Bug-K fix lives at the npm-install layer only.
- **Three GUI rows × openobserve-local — operator-driven:**
  - **claude-desktop:** PASS. `trove_harness_turn_duration_count` and `trove_harness_cost_usd` streams each show one new sample with `service_name=claude-desktop`, `harness_id=claude-desktop`. Same metrics-only path as the signoz row from `◍`; the openobserve `otlphttp` exporter accepts the Cowork-synthesised metrics without modification. The `⊕` block stays for sentry only.
  - **cursor-ide:** PASS. OpenObserve logs table returned 5 rows with `service_name=cursor`, `harness_id=cursor-cli` (`cursor.beforeShellExecution`, `cursor.afterShellExecution`, `cursor.afterAgentResponse` bodies). The Bug-L fix from `2e29e18` (cursor IDE alias in `native_service_name_candidates(CursorCli)`) holds against a fresh stack on first contact — no double-tag, no fallback to `harness.id=cursor`.
  - **cline-cli:** PASS. After `cline` (npm install at `/Users/jeff/.nvm/versions/node/v24.14.0/bin/cline`) was enabled in the Harnesses tab, a single CLI prompt landed 2 log rows + 1 `trove_harness_events` data point with `service_name=cline`, `harness_id=cline`. This proves the new `cline-cli` watcher fork below.
- **New `cline-cli` adapter — supporting Cline 3.0.13 (npm-installed):**
  - Cline ships in two distinct shapes: the VS Code extension (`globalStorage/saoudrizwan.claude-dev/tasks/<id>/ui_messages.json`, schema `[{type, text, ...}]`) and a standalone CLI 3.0.13 (`~/.cline/data/sessions/<sid>/<sid>.messages.json`, schema `{version, updated_at, agent, sessionId, messages[], system_prompt}`). The existing watcher only handled the extension tree.
  - Extended `adapters/cline_watcher.rs` to scan both trees in parallel: `scan_cli_once` walks `<sessions_dir>/<sid>/<sid>.messages.json` and reuses `cline.task_id` as the per-session key. Payloads carry an additional `cline.variant=cli|extension` attribute so downstream consumers can disambiguate without breaking the existing `cline_defaults` mapping (`say.text` → `chat.turn`).
  - `adapters/cline.rs::cli_sessions_dir` exports the new path; `detect/paths.rs` pushes it into the Cline detection candidate set; `ipc/commands.rs` updates the `HarnessId::Cline` Tier-3 spawn site to pass both directories.
  - Regression locks: 4 new unit tests in `cline_watcher.rs::tests` (`cli_watcher_emits_on_new_session_file`, `cli_watcher_parses_message_count`, `cli_watcher_skips_unknown_message_shape`, `cli_watcher_dedupes_unchanged_session`) plus a signature update in the existing `tests/cline_e2e.rs` so the extension-only fixture also exercises the new dual-tree signature.
- **`probe.rs` extension — version-manager fallback dirs (supporting Bug K end-to-end):**
  - Renamed `homebrew_fallback_dirs` → `fallback_bin_dirs` since it now covers more than Homebrew. Mac branch walks `/opt/homebrew/bin`, `/usr/local/bin`, then nvm/volta/fnm version dirs. The nvm and fnm walks are mtime-desc so a fresh node install wins on tie, matching the operator's "latest version I installed is the one I want" expectation. Volta is a single directory because the proxy handles version resolution internally.
  - The same fallback rescues every other harness that depends on a npm-installed binary from GUI-launched Trove (cline CLI, future TypeScript-based plugins). The cline-cli adapter benefits from this on first contact: `which cline` returned 1 under GUI-launchd before the fix; finds it under nvm now.
- **Bugs surfaced (none — fix-only commit):** Bug K's verification revealed an opencode-plugin-SDK quirk (one-shot CLI exit faster than the 60s metric flush) but that's an opencode/OTel-JS upstream behaviour, not a Trove bug. Logs+traces still verify the round-trip end-to-end.
- **Notes / follow-up:**
  - The cline-cli variant is now a separately-tracked harness footprint. The previously-`⊛`'d cline row is no longer "Cline isn't installed" — it's "the VS Code extension still isn't installed, but the CLI variant ships telemetry through the same `cline` harness id". Cells in the matrix grid that say `cline` PASS today are CLI-driven; the extension fork would PASS the moment VS Code + the Cline extension land on this host.
  - openobserve's `_search` API is `POST /api/<org>/_search?type={logs,metrics,traces}` with body `{"query":{"sql":..., "start_time":<us>, "end_time":<us>, "size":N}}` — note microseconds, not millis. Recording here for future query-shape reference; the existing matrix examples used the SQL-shell path which doesn't need this.
  - sentry-local GUI rows still deferred (same Kafka-restart + metrics-route blockers as `◍`). Cloud Phase E and the remaining 6 openobserve CLI cells (`cursor-cli`, `codex-cli`, `qwen`, `aider`, `copilot-cli`) are also still ahead.

### 2026-06-01 — grafana-local fill: claude-desktop + cursor-ide + cline-cli + opencode (◑)

- **Trove preset config:** All 7 backends stayed enabled in `state.json` (schemaVersion 10); signoz-local + openobserve-local were brought **down** first to honour the one-stack-at-a-time memory budget, leaving grafana-local (`grafana-cloud` preset row, endpoint `http://localhost:14320`) as the sole live destination. Stack booted from `observability/grafana-local` via `docker compose up -d`: front-door otelcol-contrib 0.152.0 on host 14319/14320 → Tempo (traces, OTLP gRPC `tempo:4317`), Loki (logs, OTLP HTTP `loki:3100/otlp`), Prometheus (metrics, remote-write). Trove build unchanged from `6ed2ad8`.
- **Smoke commands:**
  - cline-cli (Claude-driven): `cline --thinking none "say hi briefly"` → wrote `~/.cline/data/sessions/1780346005485_rqv4o/…messages.json`, tailed by the watcher.
  - opencode (Claude-driven): `OPENCODE_ENABLE_TELEMETRY=1 OPENCODE_OTLP_ENDPOINT=http://127.0.0.1:4318 OPENCODE_OTLP_PROTOCOL=http/protobuf opencode run "list the files in the current directory using a shell command, then say done"` (tool-using turn to force spans).
  - claude-desktop + cursor-ide (operator-driven): one Cowork local-agent turn in Claude Desktop; one Agent-mode action in Cursor IDE.
- **Receipt (Trove exporter `otlphttp/grafana-42520ad9` → :14320):** post-boot `sent_log_records` +88, `sent_metric_points` +235, `sent_spans` 2→8, all with 0 new `send_failed_*`. The large historical `send_failed` totals (logs 14811, metrics 31140, spans 229) exactly match signoz's old `sent` totals — backlog accrued while grafana was down, not this session. harness.ids delivered: claude-desktop, cursor(-cli), cline, opencode. **PASS.**
- **Query (sidecar on the `grafana-local_observability` network, since Loki/Tempo aren't host-published; Prometheus direct on :9091):**
  - **cline → Loki PASS:** `{service_name="cline"}` → 2 entries @ 16:33:26–28.
  - **claude-desktop → Loki PASS:** `{service_name="claude-code-desktop"}` → 9 entries @ 17:00:45–17:15:44. The Cowork tap emits logs alongside the metrics path; `service.name=claude-code-desktop` correlates to `harness.id=claude-desktop`.
  - **cursor-ide → Loki PASS:** `{service_name="cursor"}` → 8 entries @ 17:01:05–28.
  - **opencode → Tempo FAIL (delivered + ingested, not retrievable):** exporter `sent_spans` 2→8 (0 failed) and Tempo ingested them (`tempo_distributor_spans_received_total=12`, `tempo_ingester_traces_created_total=6` this boot), but `GET /api/search?q={resource.service.name="opencode"}` returns 0 over 10-min **and** 7-day windows and `/api/search/tag/resource.service.name/values` is empty — while persisted `codex-app-server` traces from prior runs _are_ listed. Tempo indexes by span start-time, so a skewed start-timestamp in opencode's OTel plugin buckets the spans outside any searchable window. No log fallback: opencode emitted 0 logs on these short turns (`{service_name="opencode"}` = 0 in Loki).
- **Bugs surfaced:**
  - **(opencode plugin, not Trove)** opencode spans un-time-searchable in Tempo (above). Watch for recurrence on hyperdx-local (ClickHouse `otel_traces` keyed by `Timestamp`) and elastic-local (APM spans keyed by `@timestamp`) — both index by span time; ingest-time stores (opensearch logs, sentry events) should still mask it.
  - **(grafana-local stack, not Trove)** otelcol-contrib 0.152.0 logs `"otlp" alias is deprecated; use "otlp_grpc"` and `"otlphttp" alias is deprecated; use "otlp_http"`. Harmless today (Loki + Tempo both ingest), but a future bump that removes the aliases will silently break grafana-local's exporters — fix-forward is a one-line rename in `observability/grafana-local/otel-collector/config.yaml`.
- **Notes / follow-up:** grafana-local persists Loki/Tempo/Prometheus volumes across boots, so queries must filter on a fresh timestamp window — the codex traces and stale `harness_id` label values are prior-run residue. claude-desktop × grafana confirms `⊕` is sentry-specific: grafana ingests the Cowork metrics fine. Next stack: opensearch-local (4 cells).

### 2026-06-02 — opensearch-local fill: claude-desktop + cursor-ide + cline-cli + opencode (◑)

- **Trove preset config:** grafana-local taken down; opensearch-local (`opensearch` preset, OTLP front door `http://localhost:14326`) the sole live destination. Booted from `observability/opensearch-local` via `docker compose up -d`: front-door otel-collector (host 14325/14326) → Data Prepper (otel trace/log/metrics sources on 21890/21892/21891) → OpenSearch 2.x node (security plugin disabled — plaintext `:9200`). Indices `logs-otel-v1-YYYY.MM.DD`, `metrics-otel-v1-YYYY.MM.DD`, `otel-v1-apm-span-NNNNNN`; date partitions are UTC. Trove build unchanged from `6ed2ad8`.
- **Smoke commands:** identical to the grafana entry (cline-cli + opencode Claude-driven; claude-desktop Cowork turn + cursor-ide Agent action operator-driven).
- **Receipt (Trove exporter `otlphttp/opensearch-abe033bf` → :14326, post-boot):** `sent_log_records` 733, `sent_metric_points` 2982, `sent_spans` 2 (`send_failed_spans` 241 — see opencode). Logs + metrics deliver cleanly; the large `send_failed` log/metric totals are backlog from the long window opensearch was down this session. **PASS** for logs + metrics.
- **Query (OpenSearch `_search` via the node container; security disabled, plaintext `:9200`):**
  - **cline → PASS:** `logs-otel-v1-2026.06.02`, `serviceName.keyword=cline` → 2 docs @ 02:01:13–15 UTC, `resource.attributes.harness@id=cline`.
  - **cursor-ide → PASS:** same index, `serviceName.keyword=cursor` → 6 docs @ 02:45:16–31 UTC, bodies `cursor.afterShellExecution` / `cursor.afterAgentResponse`, `harness@id=cursor-cli`. Also 3 `trove.harness.events` points in the metrics index.
  - **claude-desktop → PASS:** `metrics-otel-v1-2026.06.02`, `serviceName.keyword=claude-code-desktop` → fresh samples @ 02:45:45 UTC (`claude_code.cost.usage=0.27`, `claude_code.token.usage`), plus 5 `claude-desktop`-named `trove.harness.*` points. Cowork metrics path verified.
  - **opencode → Q:FAIL:** 0 docs in the logs index (no logs emitted on these turns); spans land in `otel-v1-apm-span-*` with an **empty `serviceName`** (98 span docs, 0 `serviceName` buckets) — same Data Prepper trace-mangling as `☆`, compounded by Trove's opensearch trace delivery mostly `send_failed`ing (241 vs 2 sent). No queryable opencode signal. Consistent with the cross-stack opencode pattern in `◑`.
- **Bugs surfaced:** none new. opencode×opensearch is the `☆` Data Prepper span-drop meeting opencode's no-log short turns. The boot-time `connection refused` retries to Data Prepper 21891/21892 in the front-door collector log were startup transients (sources came up ~30 s after the collector) and cleared on their own.
- **Notes / follow-up:** opensearch-local persists its indices; the `*-2026.06.02` date partition + a `time >= now-Nm` filter isolate this session. Next stack: hyperdx-local (4 cells) — watch opencode there too (ClickHouse `otel_traces` is span-time-indexed, like Tempo).

### 2026-06-02 — hyperdx-local fill: claude-desktop + cursor-ide + cline-cli + opencode (◑)

- **Trove preset config:** opensearch-local taken down; hyperdx-local / ClickStack (`clickstack` preset, OTLP front door `http://localhost:14324`, auth-gated by the stored ingestion key) the sole live destination. `docker compose up -d` reused the all-in-one `hyperdx-local-server` container (ClickHouse + app + bundled OTel collector + Mongo); persisted Mongo kept the prior ingestion key valid, so no re-onboarding was needed. UI :18080 (200); ClickHouse host :18123 is password-gated, in-container :8123 is open. Container reports `unhealthy` (strict internal healthcheck) but is fully functional; host = container = ClickHouse `now()`, no clock skew. Trove build unchanged from `6ed2ad8`.
- **Smoke commands:** identical to the prior entries.
- **Receipt (Trove exporter `otlphttp/clickstack-b9063a2b` → :14324, cumulative):** `sent_log_records` 233, `sent_metric_points` 1716, `sent_spans` 1390 — the stored key clears the 401, so logs/metrics/traces all deliver. **PASS.**
- **Query (ClickHouse `default.*` via the in-container HTTP API):**
  - **cline → PASS:** `otel_logs WHERE ServiceName='cline'` → 2 rows @ 12:35 UTC.
  - **cursor-ide → PASS:** `otel_logs WHERE ServiceName='cursor'` → 4 rows @ 12:44, bodies `cursor.before/afterShellExecution` + `afterAgentResponse`, `ResourceAttributes['harness.id']=cursor-cli`.
  - **claude-desktop → PASS:** `otel_metrics_sum` → fresh @ 12:43–44 for both `claude-code-desktop` (`claude_code.token.usage`, `claude_code.cost.usage`, `claude_code.active_time.total`) and `claude-desktop` (`trove.harness.events/tokens/cost.usd`); also 13 `claude-code-desktop` log rows. Cowork metrics path verified.
  - **opencode → Q:FAIL:** `otel_traces WHERE ServiceName='opencode'` → **0 rows even without a time filter**, while `codex-app-server` (1396 spans) and other services are present. `otel_traces` is `PARTITION BY toDate(Timestamp)` with a 30-day TTL keyed on `Timestamp` (span start-time), so opencode's skewed span start-timestamps land in a dropped/absent partition. Same opencode root cause as grafana's Tempo (`◑`); opencode emits no logs here either. No queryable signal.
- **Bugs surfaced:** none new — opencode×hyperdx is the same opencode span-timestamp issue, manifesting as a ClickHouse partition/TTL drop instead of Tempo's unsearchable bucket. The `clickstack` exporter's large historical `send_failed` totals are backlog from when hyperdx was down.
- **Notes / follow-up:** ClickHouse host port :18123 requires the `default` user password; query via the container (`localhost:8123`) or supply creds. Next stack: elastic-local (4 cells) — opencode's APM spans there are also `@timestamp`-indexed (span time), so expect the same Q:FAIL.

### 2026-06-02 — elastic-local fill: claude-desktop + cursor-ide + cline-cli + opencode (◑)

- **Trove preset config:** hyperdx-local taken down; elastic-local (`elastic` preset, OTLP front door via the APM server at `http://localhost:14328`) the sole live destination. `docker compose up -d` brought up `elastic-local-es` (single-node, `yellow`), `elastic-local-apm-server` (OTLP intake 14327/14328 → 8200), `elastic-local-kibana` (:5602); dev/anon auth (the placeholder API key in Trove suffices). APM data streams are per-service: `logs-apm.app.<svc>-default`, `metrics-apm.app.<svc>-default`, plus a shared `traces-apm-default`. ES :9200 is container-only. Trove build unchanged from `6ed2ad8`.
- **Smoke commands:** identical to the prior entries.
- **Receipt (Trove exporter `otlphttp/elastic-20e416cd` → :14328):** `sent_log_records` 1275, `sent_metric_points` 1995, `sent_spans` 478 (`send_failed_spans` 1159 — see opencode). Logs + metrics deliver cleanly. **PASS** for logs + metrics.
- **Query (ES `_search` via the `elastic-local-es` container):**
  - **cline → PASS:** `logs-apm.app.*`, `service.name=cline` → 2 docs.
  - **cursor-ide → PASS:** `logs-apm.app.cursor*`, `service.name=cursor` → 6 docs @ 13:40–41 UTC, messages `cursor.before/afterShellExecution` + `afterAgentResponse`, `labels.harness_id=cursor-cli`.
  - **claude-desktop → PASS:** `metrics-apm.app.*` shows fresh `claude-code-desktop` (21) + `claude-desktop` (5) metric docs in the last 25m (also 13 `claude-code-desktop` log docs). Cowork metrics path verified.
  - **opencode → Q:FAIL:** `traces-apm*`, `service.name=opencode` → **0 docs even without a time filter** (and 0 traces of any service in the last 25m). Trove's elastic span delivery heavily fails (`send_failed_spans` 1159 vs `sent` 478): the APM server rejects opencode's OTLP spans because their skewed start-timestamps fall outside the APM event ingest window (`@timestamp`-indexed). Same opencode root cause as `◑`; no logs to fall back on. No queryable signal.
- **Bugs surfaced:** none new — opencode×elastic is the same opencode span-timestamp issue, surfacing here as APM-server span rejection (`send_failed`) rather than a silent drop. Large historical `send_failed` log/metric totals are backlog from when elastic was down.
- **Notes / follow-up:** ES :9200 is not host-published; query via the `elastic-local-es` container or Kibana (:5602). Next stack: sentry-local (3 cells — claude-desktop is structurally blocked, `⊕`). Boots via `./scripts/up.sh`; watch the Kafka restart-loop + the OTLP-metrics 404 noted in `◍`/`⊕`.

### 2026-06-02 — sentry-local fill: cursor-ide + cline-cli + opencode (◑; claude-desktop blocked ⊕)

- **Trove preset config:** elastic-local taken down; sentry-local (self-hosted, `sentry` preset, OTLP relay `http://localhost:9000/api/2/integration/otlp/`) the sole live destination. `./scripts/up.sh` brought up the full ~30-container stack; relay/web/kafka/snuba-api came up healthy and **Kafka did not restart-loop this time** (the `★` cp-kafka 7.9.0 + `CONTROLLER` listener fix holds). The lone `metrics-consumer` restart is irrelevant (sentry doesn't ingest OTLP metrics — `⊕`). Trove build unchanged from `6ed2ad8`.
- **Smoke commands:** cline-cli + opencode Claude-driven; cursor-ide operator-driven. No claude-desktop turn — `⊕` blocked.
- **Receipt (Trove exporter `otlphttp/sentry-1d9d8eb3` → :9000):** `sent_log_records` 208, `sent_metric_points` **0** (`send_failed` 44215 — every metric 404s, the `⊕` gap), `sent_spans` 2 (`send_failed_spans` 1641). Logs deliver. **PASS** for logs.
- **Query (Snuba ClickHouse `default.eap_items_1_local` via the `sentry-self-hosted-clickhouse-1` container):** OTLP logs + spans are unified into `eap_items` (`item_type=3` logs, `item_type=1` spans); `ourlogs_*` tables stay empty on this Sentry version. Attributes are `resource.`-prefixed and sharded across 40 `attributes_string_N` maps, so query via `mapConcat(attributes_string_0..39)['resource.service.name' | 'resource.harness.id']`.
  - **cline → PASS:** `resource.service.name=cline`, `item_type=3` → 2 log rows @ 16:21 UTC, `resource.harness.id=cline`.
  - **cursor-ide → PASS:** `resource.service.name=cursor`, `item_type=3` → 4 log rows @ 16:44 UTC, bodies `cursor.before/afterShellExecution` + `afterAgentResponse`, `resource.harness.id=cursor-cli`.
  - **opencode → Q:FAIL:** absent from `eap_items` — the only `item_type=1` spans this window are `codex-app-server` (2). opencode's spans are in the 1641 `send_failed` (Sentry's relay rejects events whose timestamps fall outside its ingest window — the same skewed span start-timestamp as `◑`); opencode emits no logs. No queryable signal.
  - **claude-desktop → ⊕ (blocked, not run):** metrics-only harness; Sentry self-hosted's OTLP relay 404s `v1/metrics`. Cell stays `⊕`.
- **Bugs surfaced:** none new — opencode×sentry is the same opencode span-timestamp issue (relay rejection); the metrics 404 is the pre-existing `⊕`.
- **Notes / follow-up:** Kafka held healthy through the whole session (no relay 502s). This completes the local-stack matrix for the operator-driven GUI rows + cline-cli (PASS on every queryable local stack) and opencode (consistent `Q:FAIL` wherever the backend indexes traces by span start-time or rejects skewed timestamps; PASS only on ingest-time stores — openobserve `◇`, signoz `◔`). Remaining matrix gaps: Phase E (cloud) and the 6 openobserve CLI cells (`cursor-cli`, `codex-cli`, `qwen`, `aider`, `copilot-cli`).
