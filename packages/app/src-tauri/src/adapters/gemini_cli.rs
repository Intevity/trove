//! Gemini CLI adapter — patches `~/.gemini/settings.json`'s top-level
//! `telemetry` object. Unlike Claude Code (which uses `env`), Gemini's
//! own settings schema reserves a dedicated `telemetry` block, so the
//! managed payload is a JSON merge of nested keys under that root.
//!
//! Verify the field set against the upstream gemini-cli docs at every
//! adapter rev — the schema is still evolving as of this writing.
...
/// Resolve the absolute path of the Gemini CLI settings file under
...
/// Apply the patch. Same safety contract as `claude_code::apply`:
/// backup, atomic write, prune, and refuse to overwrite a managed
/// region whose hash differs from the new patch.
...
/// block. `customAttributes` is intentionally a no-op for Gemini in
/// Sprint 3 — the upstream schema doesn't yet expose a clear path for
/// resource attributes; we'll add the field once Gemini's docs settle.
...
mod tests {
    ...
    fn user_keys_outside_block_survive_apply() {
        ...
        // User has unrelated top-level keys.
        fs::write(
            &path,
            r#"{"theme":"dark","model":{"name":"gemini-1.5-pro"}}"#,
        )
        .unwrap();
        ...
        assert_eq!(parsed["model"]["name"], "gemini-1.5-pro");
    ...
    fn readonly_parent_dir_yields_io_error() {
        ...
        let home = tempdir().unwrap();
        let parent = home.path().join(".gemini");
