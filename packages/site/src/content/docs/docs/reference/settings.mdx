...
<Aside type="caution">
  Development builds produced by `pnpm build:app` ship **unsigned** and have auto-update disabled by
  design. If an installed copy reports it can't find updates, confirm it's a release build (from the
  [Releases page](https://github.com/Intevity/trove/releases)) and not a local dev build before
...
<Aside type="tip">
  Leave identity tagging blank if you want telemetry attributed by `harness.id` alone. You can set
  or change it at any time; new values apply to telemetry forwarded after you save.
</Aside>
...
<Aside type="caution">
  Don't hand-edit `state.json` to fit a stale binary — in particular, never bump its `schemaVersion`
  by hand. A schema change must come from a rebuilt app; editing the file to match a mismatched
  version corrupts state.
</Aside>
