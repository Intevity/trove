...
<Aside type="note">
  **The one-sentence version:** Trove's bundled collector binds to `127.0.0.1`, forwards only to the
  endpoint you configured, stores credentials in your OS keychain, and every file it touches is
  reversible byte-for-byte. It never phones home.
</Aside>
...
<Aside type="tip">
  Want to verify with your own eyes? Point Trove at a **Local Collector (passthrough)** backend, or
  watch the **Logs** tab — the live collector tail shows exactly which exporters are firing and
  where.
</Aside>
...
Credentials are **never** written to plaintext JSON, **never** placed in env files, and **never** logged. Trove's single state file (`state.json`) records _that_ a backend is configured and _what_ was patched — not the secret itself.
