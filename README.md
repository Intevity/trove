...
<p align="center"><img src="documentation/screenshots/troveOverviewAnimated.gif" alt="Trove Overview tab — live data flow from harnesses through the collector to configured platforms" width="780" /></p>

---

## Why Trove

AI coding tools have a telemetry problem: **every vendor invented their own dialect, every install hides the wiring in a different file, and the data either stays trapped in their proprietary dashboard or gets handed straight to them as a side effect of you using their CLI.** Trove flips that around. You point each tool at a local Trove collector once, and from then on every signal your team generates lands in a backend you control — typed consistently, attributed by harness, and auditable end-to-end.

That matters to different people for very different reasons:

#### 🔧 For engineering leads

- **One pane of glass across mixed-harness teams.** Half the team uses Claude Code, the other half is on Cursor + Copilot CLI, your platform team trialed Codex last week. Without Trove, that's four separate vendor dashboards (or four blanks). With Trove, every span/metric/log carries a `harness.id` resource attribute so you can compare them side-by-side in one query.
- **Native OTel where it exists, best-effort where it doesn't.** Claude Code, Gemini CLI, Codex, Qwen, OpenCode, and Cursor ship native OTLP — Trove flips the right flags and routes them through. Cline, Aider, and Copilot CLI don't natively emit OTel; Trove installs lightweight watchers and shell-rc wrappers that derive equivalent OTLP records from their on-disk logs, so they show up in the same queries as their native peers.
- **Reversible by design.** Every "Enable" writes a sentinel-bracketed managed region into the harness's config file. One click reverts it byte-for-byte. No half-applied states, no orphaned env vars, no "I uninstalled Trove but my CLI still POSTs to localhost."

#### 💸 For finance, ops, and platform owners

- **Detect dead seats before renewal.** If your org pays for Cursor + Copilot + Claude Code + Codex licenses across 200 engineers, Trove tells you which seats actually fire — by user, by week, by tool. The same `harness.id`-keyed metric stream that powers the engineering dashboard also surfaces "this license has zero turns in the last 30 days" rows that procurement can act on.
- **Cross-vendor cost normalization.** Token counts, model-call counts, and turn durations all flow through the same Tier-A metric schema (`trove.harness.tokens`, `trove.harness.events`, `trove.harness.cost.usd`, `trove.harness.turn.duration`, `trove.harness.errors`). Cost per turn for Claude Code is comparable to cost per turn for Copilot CLI in your own dashboard — no vendor-specific exporter to maintain.
- **Your contract, your data residency.** Telemetry leaves the machine only toward the backend _you_ configured. Trove never phones home, never aggregates user data, never has a SaaS layer that could change its mind about pricing or data-sharing policies next quarter.

#### 🔒 For security and IT

- **Localhost-only by design.** The bundled OpenTelemetry Collector listens on `127.0.0.1` and forwards exclusively to the endpoint you set. No third-party SDK in the dependency tree phones a vendor.
- **OS-keychain credential storage.** Backend tokens, API keys, ingest secrets — all stored in macOS Keychain, Windows Credential Manager, or Linux Secret Service. Never in plaintext JSON, never in env files, never logged.
...
- **MIT-licensed, fully open source.** Every line of TypeScript, every Rust handler, every adapter — auditable. CI gates ≥ 95% test coverage on the shared schema layer where security-sensitive logic lives.
...
<p align="center"><img src="documentation/screenshots/harnesses.png" alt="Harnesses tab — auto-detected AI coding tools with per-row Enable/Disable toggles, telemetry status pills, and coverage badges" width="780" /></p>
...
| Native OTel      | Cursor IDE             | Cursor hooks → `~/.cursor/hooks.json`             |
...
| Best effort      | Aider                  | Shell-rc wrapper tees session log → OTLP          |
| Best effort      | GitHub Copilot CLI     | Shell-rc wrappers around `copilot` / `gh copilot` |
...
When a harness is enabled, Trove shows it in the Overview Data flow chart (≤ 3 tools = individual nodes; 4+ collapses into an animated "Orbital Hub" cluster) with per-source activity halos that light up when telemetry is flowing.
...
<p align="center"><img src="documentation/screenshots/platforms.png" alt="Platforms tab — supported backends with per-instance Add / Edit / Disable / Remove controls, recommended ordering, and health status" width="780" /></p>

| Platform                         | Auth pattern                           |
| -------------------------------- | -------------------------------------- |
| **SigNoz Cloud** _(recommended)_ | Ingestion key (gRPC or HTTP)           |
| Grafana Cloud                    | Endpoint + Basic auth                  |
| Honeycomb                        | Team API key + dataset                 |
| Datadog                          | DD-API-KEY header + site selector      |
| New Relic                        | License key + region (US / EU)         |
| Splunk Observability Cloud       | Realm + `X-SF-Token` access token      |
| Dynatrace                        | Environment URL + API token            |
| Elastic                          | OTLP/HTTP + `Authorization: ApiKey`    |
| OpenSearch                       | OTLP → Data Prepper                    |
| OpenObserve                      | OTLP + `/api/<organization>` path      |
| ClickStack (HyperDX)             | OTLP + ingestion key header            |
| Chronosphere                     | Tenant + `API-Token` header            |
| Sentry                           | OTLP/HTTP + `X-Sentry-Auth`            |
| Generic OTLP                     | Arbitrary endpoint + any header set    |
| Local Collector (passthrough)    | OTLP to your own self-hosted collector |

**Multi-platform fan-out is the default.** Trove broadcasts every signal to every enabled platform — there's no per-platform routing. Configure SigNoz + Honeycomb + Generic OTLP all at once and every harness's telemetry lands in all three. Per-platform **Disable** lets you pause forwarding without losing credentials; re-enable with one click and the collector picks up where it left off.
...
<p align="center"><img src="documentation/screenshots/mappings.png" alt="Mappings tab — per-harness synthesis and hook rules feeding the Tier-A metric schema, with visual mapping editor and full-diff preview" width="780" /></p>

What you can do from the Mappings tab:

- **Synthesis rules** — turn a raw harness counter (e.g. `claude_code.session.count`) into a Tier-A metric (`trove.harness.events`) with optional attribute filters. Rename type → direction, fan a single source into multiple metrics, or alias attribute keys.
...
- **Apply lives.** Mappings are evaluated by the collector itself (`metricstransform` + `transform/harness-tag` overlays), so changes take effect on the next reload — no restart, no waiting for an agent to pick up new config.
...
> **macOS note:** pre-1.0 builds ship unsigned. The first launch needs a one-time right-click → **Open** to clear Gatekeeper.
...
- **Tauri 2** Rust shell with a React + TypeScript WebView UI. ~30 MB final bundle.
