#!/bin/sh
# Trove's Antigravity CLI (`agy`) hook — Node-resolution shim.
#
# Antigravity invokes this file on every registered agent event
# (UserPromptSubmit / Stop / BeforeShellExecution / AfterShellExecution /
# ErrorOccurred). The actual hook logic is JavaScript and lives in the
# sibling file `antigravity-otel-hook-impl.cjs`; this wrapper exists
# because macOS GUI apps launched from the Dock or Spotlight inherit
...
#
# Keep this file at the path the antigravity adapter
# (antigravity_cli.rs::build_region) records in
# ~/.gemini/antigravity-cli/hooks.json. The kernel reads the shebang below
# and runs /bin/sh regardless of the file's .cjs extension.
#
...
IMPL="$SELF_DIR/antigravity-otel-hook-impl.cjs"

# Diagnostic trace so we can confirm Antigravity *does* invoke this
# wrapper — the file is the first thing we look at when "I don't see any
# antigravity metrics in SigNoz" comes back. Best-effort: every step
# swallows IO errors with `|| true` so a missing log dir or read-only
# home cannot crash the hook. The log path mirrors the collector log
# directory (~/Library/Logs/com.intevity.trove on macOS).
TROVE_HOOK_LOG="${HOME:-/tmp}/Library/Logs/com.intevity.trove/antigravity-hook-invocations.log"
...
NODE_BIN=$(resolve_node) || {
  ...
  # No node anywhere on the system. Never block the user's session: a
  # `command` hook that exits 0 with empty stdout is the protocol-agnostic
  # "proceed" signal, so we stay silent and let agy fall through to its
  # default (allow) for gate events like BeforeShellExecution.
  exit 0
}
